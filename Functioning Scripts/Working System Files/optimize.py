...
def read_data():
    ...
    body = [r for r in body if any(field.strip() for field in r)]
    ...
    if body:
        ...
        # Convert to float immediately during extraction to avoid string/float casting errors
        def to_f(val):
            try: return float(val)
            except: return 0.0

        databucketX = [[to_f(row[j]) if j < len(row) else 0.0 for row in body] for j in range(1, 1 + ncols)]
        databucketY = [to_f(row[-1]) for row in body]
    else:
        databucketX = []
        databucketY = []
...
def scale_array(array, bounds):
    """
    General purpose linear scaling function.
    Maps an array from its own [min, max] to the specified bounds.
    """
    arr = np.array(array, dtype=float)
    
    # Check if bounds is a single pair [low, high] or a list of pairs [[l1,h1], [l2,h2]...]
    # If it's a list/tuple where the first element is a number, wrap it.
...
def denormalize(array, bounds):
    """
    Maps a normalized array (range [-1, 1]) back to physical units.
    Uses scale_array three times, once for each dimension.
    """
    arr = np.array(array, dtype=float)
    is_1d = (arr.ndim == 1)
    
    # Reshape to 2D if 1D for consistent indexing
    if is_1d:
        data = arr.reshape(1, -1)
    else:
        data = arr
        
    # Apply scale_array 3 times (for 3 dimensions)
    # Note: scale_array maps internal [min, max] to bounds.
    # To force [-1, 1] -> [Low, High], we ensure the input column has -1 and 1
    # or we accept that it scales the distribution relative to itself.
    col1 = scale_array(data[:, 0], bounds[0])
    col2 = scale_array(data[:, 1], bounds[1])
    col3 = scale_array(data[:, 2], bounds[2])
    
    # Reconstruct
    out = np.column_stack((col1, col2, col3))
    
    return out[0].tolist() if is_1d else out.tolist()
...
def train_weights(X_bucket, Y_bucket, lambda_reg=1e-8):
    """
    Calculates the weights (omegas) for a radial basis function surrogate model.
    
    Attempts to solve Phi * w = Y normally. If the matrix is singular, 
    applies Tikhonov Regularization (Phi + lambda*I) as a fallback.
    """
    Y = np.array(Y_bucket)
    ...
    try:
        # Step 1: Attempt standard solve (Maximum accuracy)
        omegas = np.linalg.solve(Phi, Y)
    except np.linalg.LinAlgError:
        # Step 2: Fallback to Regularization if matrix is singular/ill-conditioned
        print("Matrix is singular. Applying Tikhonov Regularization...")
        Phi_reg = Phi + np.eye(Phi.shape[0]) * lambda_reg
        try:
            omegas = np.linalg.solve(Phi_reg, Y)
        except np.linalg.LinAlgError:
            # Step 3: Ultimate fallback to pseudo-inverse
            print("Regularized solve failed. Falling back to pseudo-inverse.")
            omegas = np.linalg.pinv(Phi_reg).dot(Y)
